...
static auto kRoundOptionsType = GetFunctionOptionsType<RoundOptions>(
    ...
    static auto kRoundBinaryOptionsType = GetFunctionOptionsType<RoundBinaryOptions>(
            DataMember("ndigits", &RoundBinaryOptions::ndigits),  // MEGAHACK -- Remove.
...
constexpr char RoundOptions::kTypeName[];
        ...
        RoundBinaryOptions::RoundBinaryOptions(int64_t ndigits, RoundMode round_mode)
                : FunctionOptions(internal::kRoundBinaryOptionsType),
                  ndigits(ndigits),
                  round_mode(round_mode) {
            static_assert(RoundMode::HALF_DOWN > RoundMode::DOWN &&
                          RoundMode::HALF_DOWN > RoundMode::UP &&
...
Result<Datum> RoundBinary(const Datum& arg1, const Datum& arg2, RoundBinaryOptions options, ExecContext* ctx) {
